<template>
	<view class="picture-tailor" :class="{'picture-tailor-show':isShow}">
		...
		<canvas style="position: absolute;bottom: -1000px;" canvas-id="picture-canvas" class="canvas-view">
		</canvas>
	</view>
</template>
						...
						console.log(res.tempFilePath)
						 uni.uploadFile({
						            ...
						            success: (uploadFileRes) => {
										...
										let reslut = JSON.parse(uploadFileRes.data).data[0].url
										console.log(reslut)
										this.isShow = false;
										this.$emit("createImg", reslut)
						            }
						        });
